...
namespace BankingSimulation.MBNA.Services.Orchestration
{
            ...
            ITransactionImportOrchestrationService
    {
        ...
        {
            await CreateMBNASystem();

            var parsedAccounts = accountProcessingService.ParseAccounts();

            var parsedAccountNumbers = parsedAccounts.Select(pa => pa.Number).ToArray();

            var existingAccounts = foundationService
                .GetAll<Account>()
                .Where(a => a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "MBNA") && parsedAccountNumbers.Contains(a.Number))
                .ToArray();
            ...
            foreach (var entry in newAccounts)
            {
                ...
                await foundationService.AddAsync(new AccountBankingSystemReference { BankingSystemId = "MBNA", AccountId = topLevelAccount.Id });
        ...
        public async Task ImportTransactionsFromRawDataAsync(string rawData)
        {
            ...
            var existingAccounts = foundationService.GetAll<Account>()
                .Where(a => receivedAccountNumbers.Contains(a.Number)
                    && a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "MBNA")
                )
            ...
            var existingTransactionTypes = foundationService
                ...
                .Where(tt => tt.SystemId == "MBNA" && transactionTypes.Contains(tt.TypeId))
                .Select(tt => tt.TypeId)
                .ToArray();
            ...
            foreach (var group in entriesForDate)
            {
                ...
                var existingTransactionsForDate = foundationService
                    .GetAll<Transaction>()
                    .Where(t => t.Date == group.Key && receivedAccountNumbers.Contains(t.Account.Number) && t.SourceSystemId == "MBNA")
                    .Include(t => t.Account)
