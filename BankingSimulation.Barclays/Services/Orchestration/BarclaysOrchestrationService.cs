        ...
        ITransactionImportOrchestrationService
{
    ...
    {
        if (!foundationService.GetAll<BankingSystem>().Any(bs => bs.Id == "Barclays"))
            await foundationService.AddAsync(new BankingSystem { Id = "Barclays" });
    }

    public async Task ImportAccountsFromRawDataAsync(string rawData)
    {
        await CreateBarclaysSystem();
        ...
        var existingAccounts = foundationService
            .GetAll<Account>()
            .Where(a => a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "Barclays") && parsedAccountNumbers.Contains(a.Number))
            .ToArray();
        ...
        foreach (var entry in newAccounts)
        {
            ...
            await foundationService.AddAsync(new AccountBankingSystemReference { BankingSystemId = "Barclays", AccountId = topLevelAccount.Id });
    ...
    public async Task ImportTransactionsFromRawDataAsync(string rawData)
    {
        ...
        var existingAccounts = foundationService.GetAll<Account>()
            .Where(a => receivedAccountNumbers.Contains(a.Number)
                && a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "Barclays")
            )
        ...
        var existingTransactionTypes = foundationService
            ...
            .Where(tt => tt.SystemId == "Barclays" && transactionTypes.Contains(tt.TypeId))
            .Select(tt => tt.TypeId)
            .ToArray();
        ...
        foreach (var group in entriesForDate)
        {
            ...
            var existingTransactionsForDate = foundationService
                .GetAll<Transaction>()
                .Where(t => t.Date == group.Key && receivedAccountNumbers.Contains(t.Account.Number) && t.SourceSystemId == "Barclays")
                .Include(t => t.Account)
