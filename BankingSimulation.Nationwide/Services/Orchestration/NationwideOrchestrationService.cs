        ...
        ITransactionImportOrchestrationService
{
    ...
    {
        if (!foundationService.GetAll<BankingSystem>().Any(bs => bs.Id == "Nationwide"))
            await foundationService.AddAsync(new BankingSystem { Id = "Nationwide" });
    }

    public async Task ImportAccountsFromRawDataAsync(string rawData)
    {
        await CreateNationwideSystem();
        ...
        var existingAccounts = foundationService
            .GetAll<Account>()
            .Where(a => a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "Nationwide") && parsedAccountNumbers.Contains(a.Number))
            .ToArray();
        ...
        foreach (var entry in newAccounts)
        {
            ...
            await foundationService.AddAsync(new AccountBankingSystemReference { BankingSystemId = "Nationwide", AccountId = topLevelAccount.Id });
    ...
    public async Task ImportTransactionsFromRawDataAsync(string rawData)
    {
        ...
        var existingAccounts = foundationService.GetAll<Account>()
            .Where(a => receivedAccountNumbers.Contains(a.Number)
                && a.AccountSystemReferences.Any(asr => asr.BankingSystemId == "Nationwide")
            )
        ...
        var existingTransactionTypes = foundationService
            ...
            .Where(tt => tt.SystemId == "Nationwide" && transactionTypes.Contains(tt.TypeId))
            .Select(tt => tt.TypeId)
            .ToArray();
        ...
        foreach (var group in entriesForDate)
        {
            ...
            var existingTransactionsForDate = foundationService
                .GetAll<Transaction>()
                .Where(t => t.Date == group.Key && receivedAccountNumbers.Contains(t.Account.Number) && t.SourceSystemId == "Nationwide")
                .Include(t => t.Account)
